...
class HeterogeneousZeROOptimizer(BaseOptimizerWrapper):
    """A wrapper for optimizer. ``_FullyShardedDataParallel`` and ``ZeroOptimizer`` implement Zero Redundancy Optimizer (ZeRO state-3).

    Note:
        You must use ``_FullyShardedDataParallel`` with ``ZeroOptimizer``.
    ...
    Args:
        ...
        module (_FullyShardedDataParallel): A ``_FullyShardedDataParallel`` instance.
        gpu_margin_mem_ratio (float, optional): The ratio of GPU remaining memory (after the first forward-backward)
            which will be used when using hybrid CPU optimizer.
            This argument is meaningless when `placement_policy` of `GeminiManager` is not "auto".
            Defaults to 0.0.
        clipping_norm (float, optional): The norm value to clip the gradient. Defaults to 0.0.
        norm_type (float, optional): The type of norm to clip the gradient. Defaults to 2.0.
        num_fp32_shards_per_param(int, optional): The number of fp32 shards per parameter. Defaults to 0.
    """
    ...
    def __init__(
        ...
        **defaults: Any,
        ...
        self._num_fp32_shards_per_param = num_fp32_shards_per_param
    ...
    def _get_combined_scale(self):
        ...
        if self.clipping_flag:
            ...
            clip = (total_norm + 1e-6) / self.max_norm
            if clip > 1:
                combined_scale = clip
        if combined_scale == 1:
            return -1
    ...
    def _maybe_move_fp32_params(self):
        if self._should_move_fp32_params_h2d:
            ...
            fp32_params_available_cuda_margin_mem = (
                available_cuda_margin_mem / self._num_fp32_shards_per_param
            )
